from secrets import choice
from replit import clear
clear()

def asci_prints():
    print('''
    *******************************************************************************
            |                   |                  |                     |
    _________|________________.=""_;=.______________|_____________________|_______
    |                   |  ,-"_,=""     `"=.|                  |
    |___________________|__"=._o`"-._        `"=.______________|___________________
            |                `"=._o`"=._      _`"=._                     |
    _________|_____________________:=._o "=._."_.-="'"=.__________________|_______
    |                   |    __.--" , ; `"=._o." ,-"""-._ ".   |
    |___________________|_._"  ,. .` ` `` ,  `"-._"-._   ". '__|___________________
            |           |o`"=._` , "` `; .". ,  "-._"-._; ;              |
    _________|___________| ;`-.o`"=._; ." ` '`."\` . "-._ /_______________|_______
    |                   | |o;    `"-.o`"=._``  '` " ,__.--o;   |
    |___________________|_| ;     (#) `-.o `"=.`_.--"_o.-; ;___|___________________
    ____/______/______/___|o;._    "      `".o|o_.--"    ;o;____/______/______/____
    /______/______/______/_"=._o--._        ; | ;        ; ;/______/______/______/_
    ____/______/______/______/__"=._o--._   ;o|o;     _._;o;____/______/______/____
    /______/______/______/______/____"=._o._; | ;_.--"o.--"_/______/______/______/_
    ____/______/______/______/______/_____"=.o|o_.--""___/______/______/______/____
    /______/______/______/______/______/______/______/______/______/______/_____ /
    *******************************************************************************
    ''')
    print("Welcome to Treasure Island.")
    print("Your mission is to find the treasure.") 

#https://www.draw.io/?lightbox=1&highlight=0000ff&edit=_blank&layers=1&nav=1&title=Treasure%20Island%20Conditional.drawio#Uhttps%3A%2F%2Fdrive.google.com%2Fuc%3Fid%3D1oDe4ehjWZipYRsVfeAx2HyB7LCQ8_Fvi%26export%3Ddownload

#Write your code below this line 👇
def thirds(color):
    if color == "red": 
        return ("You died by fire breathing dragon! Soz")
    elif color == "blue":
        return ("You were burried alive, and i oops.")
    elif color == "yellow":
        return ("That's my mom's favorite color therefore, you won! :) KK bye")
    else: 
        return("That's not a choice lol you lost big time bye! ")

adventure = True 

while adventure != False: 
    clear()
    asci_prints()
    first_choice = input(f"\nYou're at a cross road. Where do you want to go?\nType \"left\" or \"right\" ").lower()
    if first_choice == "left":
        second_choice = input(f"\nYou come to a lake. There is an island in the middle of the lake. Type \"wait\" to wait for a boat. Type \"swim\" to swim across. ").lower()
        if second_choice == "swim":
            third_choice = input(f"\nYou arrive at the island unharmed. THere is a house with 3 doors. One \"red\", one \"yellow\" and one \"blue\". Which color do you choose? ").lower()
            thirds(color=third_choice)
        else: 
            print("Game over homes! You are out of here!")
    else:
        print("\n\nA shark ate you! Ahhhhhhh")
    retry_adventure = input(f"\nWould you like to try again? \"yes\" or \"no\": ").lower()
    if retry_adventure != "yes":
        adventure = False
    